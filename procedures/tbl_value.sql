--tbl_value.sql
--create
CREATE OR ALTER PROCEDURE [dbo].[createTblValue]
    @value varchar(max),
...
BEGIN
    BEGIN TRY
        IF @tranCount = 0
            BEGIN TRANSACTION
        ELSE
            SAVE TRANSACTION [createTblValue];
    ...
    BEGIN CATCH
        ...
        IF @xstate = 1 AND @tranCount > 0
            ROLLBACK TRANSACTION [createTblValue];
...
--update
CREATE OR ALTER PROCEDURE [dbo].[updateTblValue]
    @value varchar(max),
    @productId int,
    @attributeId int,
    @idOut int = NULL OUTPUT
...
BEGIN
    BEGIN TRY
        IF @tranCount = 0
            BEGIN TRANSACTION
        ELSE
            SAVE TRANSACTION [updateTblValue];
            DECLARE @errors varchar(max);
    ...
    BEGIN CATCH
        ...
        IF @xstate = 1 AND @tranCount > 0
            ROLLBACK TRANSACTION [updateTblValue];
...
--delete
CREATE OR ALTER PROCEDURE [dbo].[deleteTblValue]
    @productId int,
    @attributeId int,
    @idOut int = NULL OUTPUT
AS
...
BEGIN
    BEGIN TRY
        IF @tranCount = 0
            BEGIN TRANSACTION
        ELSE
            SAVE TRANSACTION [deleteTblValue];
            DECLARE @errors varchar(max);
    ...
    BEGIN CATCH
        ...
        IF @xstate = 1
            ROLLBACK;
        IF @xstate = 1 AND @tranCount > 0
            ROLLBACK TRANSACTION [deleteTblValue];
        SELECT 0 AS affects_rows, @message AS error, NULL AS id;
    END CATCH
END
GO