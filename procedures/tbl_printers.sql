...
BEGIN
    ...
    WHERE ip = @ip)
        ...
        INSERT INTO 
                    tbl_printers
            (dateCreated, dateModified, ip, model, locationId, protocol, alias)
        OUTPUT inserted.id INTO @id
        VALUES
            (GETDATE(), GETDATE(), @ip, @model, @locationId, @protocol, @alias);
        SELECT TOP 1
            @idOut = id
        FROM @id;
...
BEGIN
    ...
    WHERE id = @deviceId)
        ...
        ELSE IF (@ip IS NOT NULL AND @currentIp = @ip)
            SET @errors = CONCAT(@errors, 'La IP no puede ser la misma: ', @ip, CHAR(13), CHAR(10));

        ELSE IF (@ip IS NOT NULL AND @currentIp <> @ip AND EXISTS(SELECT ip
        FROM tbl_printers
        WHERE ip = @ip))
            SET @errors = CONCAT(@errors, 'IP ya registrada: ', @ip, CHAR(13), CHAR(10));
        ...
        UPDATE 
                ...
                WHERE 
                    id = @deviceId;
        SELECT 1 AS affects_rows, NULL AS error, @deviceId AS id;
    END
        ELSE
            BEGIN
        SELECT 0 AS affects_rows, @errors AS error, @deviceId AS id;
    END;
        lbexit:
            IF @tranCount = 0
            COMMIT;
